...
# TODO: Once we have relationships between predictors

# Let's explore some of the relationships between some of our predictors.
...
TestExps = function(pct, data, target, exps) {
  ...
  errors = mapply(AbsError, model = models, predictor = colnames(data), MoreArgs = list(data = split$testData, target = split$testTarget))
  return(errors)
}
